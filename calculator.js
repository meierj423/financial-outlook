document.addEventListener("DOMContentLoaded", function() {
    ...
    form.addEventListener('submit', function (e) {
        ...
        calculate(); // Assuming you have a function called calculateMortgage for your calculations
    });

    // Retrieve form data from localStorage on load
    function populateForm() {
        const formElements = form.elements;
        for (let i = 0; i < formElements.length; i++) {
            const name = formElements[i].name;
            if (localStorage.getItem(name)) {
                formElements[i].value = localStorage.getItem(name);
            }
        }
    }
    ...
    // Handle the "Back to Calculator" button click
